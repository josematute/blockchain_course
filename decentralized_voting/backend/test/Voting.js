...
describe("Voting", function () {
	...
	describe("Create vote", () => {
		it("Cannot create vote if not member", async () => {
			await expect(voting.connect(addr1).createVote("", (await getTime()) + 60, 4)).to.be.reverted
		})
		it("Cannot create vote invalid end time", async () => {
			await expect(voting.createVote("", (await getTime()) - 60, 4)).to.be.reverted
		})
		it("Cannot create vote it too few options", async () => {
			await expect(voting.createVote("", (await getTime()) + 60, 1)).to.be.reverted
		})
		it("Cannot create vote if too many options", async () => {
			await expect(voting.createVote("", (await getTime()) + 60, 9)).to.be.reverted
		})
		it("Can create vote", async () => {
			await expect(voting.createVote("1", (await getTime()) + 60, 3)).to.emit(voting, "VoteCreated")
		})
		it("Can create 2nd vote", async () => {
			await expect(voting.createVote("2", (await getTime()) + 60, 4)).to.emit(voting, "VoteCreated")
		})
	})
