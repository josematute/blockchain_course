...
contract BankAccount {
    ...
    function createAccount(
        ...
        // copy passed in array to owners
        for (uint idx; idx < owners.length; idx++) {
            if (idx < owners.length - 1) {
                owners[idx] = otherOwners[idx];
            }
            // add user to userAccounts, can only have 3 max accounts
    ...
    function withdraw(
        ...
        accounts[accountId].balance -= amount; // important to do this before to avoid attacks
        // reset all the withdrawRequest values to default values
